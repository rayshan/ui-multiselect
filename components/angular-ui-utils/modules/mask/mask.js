...
angular.module('ui.mask',[])
  ...
  compile: function uiMaskCompilingFunction(){
    ...
    return function uiMaskLinkingFunction(scope, iElement, iAttrs, controller){
      ...
      var // Minimum required length of the value to be considered valid
      minRequiredLength;
      ...
      var // Vars for initializing/uninitializing
      originalPlaceholder = iAttrs.placeholder;

      var originalMaxlength = iAttrs.maxlength;

      var // Vars used exclusively in eventHandler()
      oldValue;
      ...
      function processRawMask(mask){
        ...
        if (typeof mask === 'string') {
          ...
          angular.forEach(mask.split(''), chr => {
      ...
      function eventHandler(e){
        ...
        var // Case: Typing a character to overwrite a selection
        isAddition = (val.length > valOld.length) || (selectionLenOld && val.length > valOld.length - selectionLenOld);

        var // Case: Delete and backspace behave identically on a selection
        ...
        var // Necessary due to "input" event not providing a key code
        isKeyBackspace = eventWhich === 8 || (eventType !== 'keyup' && isDeletion && (caretPosDelta === -1));

        var isKeyDelete = eventWhich === 46 || (eventType !== 'keyup' && isDeletion && (caretPosDelta === 0 ) && !wasSelected);

        var // Handles cases where caret is moved and placed in front of invalid maskCaretMap position. Logic below
